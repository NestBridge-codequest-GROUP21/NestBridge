...
import CulturalQuizScreen from '../screens/onboarding/CulturalQuizScreen';
...
export type StudentEntryStackParamList = {
  ...
  CulturalQuiz: { questionIndex: number };
        ...
        <Stack.Screen name="Register">
          {({ navigation }) => (
            <RegisterScreen
              ...
              onSignInPress={() => navigation.navigate('Destination')}
          ...
          {({ navigation }) => (
            <DestinationSetupScreen
              ...
              onSelectSuggestion={(id) => {
                ...
                const match = destinationSuggestions.find((s) => s.id === id);
                if (match) {
                  setCity(`${match.city}, ${match.country}`);
                }
              }}
              onContinue={() => navigation.navigate('CulturalQuiz', { questionIndex: 0 })}
              onBack={() => navigation.goBack()}
            />
          )}
        </Stack.Screen>

        <Stack.Screen name="CulturalQuiz">
          {({ navigation, route }) => {
            const { questionIndex } = route.params;
            const question = culturalQuizQuestions[questionIndex];
            ...
            return (
              <CulturalQuizScreen
                ...
                questionTotal={culturalQuizQuestions.length}
                question={question.question}
                helperText={question.helperText}
                options={question.options}
                selectedOptionId={selectedOptionId}
                onSelectOption={(optionId) =>
                  setQuizAnswers((prev) => ({ ...prev, [question.id]: optionId }))
                }
                onContinue={() => {
                  if (questionIndex < culturalQuizQuestions.length - 1) {
                    navigation.push('CulturalQuiz', { questionIndex: questionIndex + 1 });
                  } else {
                ...
                onBack={() => {
                  if (questionIndex > 0) {
                    navigation.replace('CulturalQuiz', { questionIndex: questionIndex - 1 });
                  } else {
                    navigation.goBack();
                  }
                }}
